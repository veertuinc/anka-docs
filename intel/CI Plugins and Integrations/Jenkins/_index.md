...
3. On the host, enable SSH [port forwarding]({{< relref "intel/Anka Virtualization/command-reference.md#modify-vmnameoruuid-add-port-forwarding" >}}) for your VM Template using the Anka CLI: `sudo anka modify $SOURCE_TEMPLATE add port-forwarding --guest-port 22 ssh`. _We recommend not specifying --host-port._
