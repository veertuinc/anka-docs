...
{{< hint info >}}
Before upgrading, check if your current version is noted in the [Pre-Upgrade Considerations]({{< archRelRef "Anka Build Cloud/upgrading.md#pre-upgrade-considerations" >}}) and adjust your upgrade plan accordingly.
{{< /hint >}}
...
If necessary, you can [force the proper agent version task creation through the Controller API.]({{< archRelRef "Anka Build Cloud/working-with-controller-and-API.md#force-node-agent-update" >}}). Alternatively you can also disjoin the Nodes first, do the upgrade of the Controller, and then manually execute `curl -O http://**{controllerUrlHere}**/pkg/AnkaAgent.pkg && sudo installer -pkg AnkaAgent.pkg -tgt /` on each node individually.
{{< /hint >}}
...
{{< hint info >}}
We recommend [snapshotting your etcd database]({{< archRelRef "Anka Build Cloud/setup-on-linux-with-docker.md#etcd-snapshotting" >}}) regularly, but especially before an upgrade.
{{< /hint >}}
...
#### Docker
  ...
  2. [Download and extract the latest package]({{< archRelRef "Anka Build Cloud/setup-on-linux-with-docker.md#step-2-install-the-anka-build-cloud-controller--registry" >}}).
...
#### Native macOS package
  ...
  2. Install the new .pkg (see the [MacOS Guide]({{< archRelRef "Anka Build Cloud/setup-on-macos.md" >}})).
